    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < Square.Length; i++)
            {
                ...
                Square[i] = pyr[i].Sp(pyr[i].getA(),pyr[i].getP());
                Console.WriteLine(pyr[i].ToStr());
            }
            Console.WriteLine("\n");
            for (int i = 0; i < Volume.Length; i++)
            {
                cyl[i] = new TCylinder(Rnd(), Rnd());
                Volume[i] = cyl[i].Vc(cyl[i].getR(),cyl[i].getH());
                Console.WriteLine(cyl[i].ToStr());
            }
            ...
            Console.WriteLine($"min Pyramid Square = {Math.Round(min, 3)}");
            Console.WriteLine($"max Cylinder Volume = {Math.Round(max, 3)}");
